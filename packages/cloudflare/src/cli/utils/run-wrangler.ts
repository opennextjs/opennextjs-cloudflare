...
/**
 * Runs a wrangler command and captures the output.
 * The output is both printed to the console and returned.
 *
 * @param options Build options
 * @param args Command arguments
 * @param wranglerOpts Wrangler options
 * @returns The combined stdout and stderr output
 */
export function runWranglerCapture(
	...
	// Print output to console if logging is enabled
	if (wranglerOpts.logging === "all") {
		process.stdout.write(stdout);
		process.stderr.write(stderr);
	} else if (wranglerOpts.logging === "error") {
		process.stderr.write(stderr);
	}

	if (result.status !== 0) {
		if (wranglerOpts.logging !== "all") {
			// Print stdout on error if not already printed
			process.stdout.write(stdout);
		}
		logger.error("Wrangler command failed");
		process.exit(1);
