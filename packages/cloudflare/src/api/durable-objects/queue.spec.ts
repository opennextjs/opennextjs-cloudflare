...
const createDurableObjectQueue = ({fetchDuration, statusCode, headers}: { fetchDuration: number, statusCode?: number, headers?: Headers }) => {
  const mockState = {
    waitUntil: vi.fn(),
  ...
  return new DurableObjectQueueHandler(mockState as any, {
    NEXT_CACHE_REVALIDATION_WORKER: {
      fetch: vi.fn().mockReturnValue(new Promise<Response>((res) => setTimeout(() => res(new Response(null, {
        status: statusCode,
...
describe('DurableObjectQueue', () => {
  ...
  describe('alarm', () => {
    it('should execute revalidations for expired events', async () => {
      const queue = createDurableObjectQueue({fetchDuration: 10});
      queue.routeInFailedState.set("id", {msg: createMessage("id"), retryCount: 0, nextAlarm: Date.now()-1000});
      queue.routeInFailedState.set("id2", {msg: createMessage("id2"), retryCount: 0, nextAlarm: Date.now()-1000});
      await queue.alarm();
      expect(queue.routeInFailedState.size).toBe(0);
      expect(queue.service.fetch).toHaveBeenCalledTimes(2);
    });
    ...
    it('should execute revalidations for the next event to retry', async () => {
      const queue = createDurableObjectQueue({fetchDuration: 10});
      queue.routeInFailedState.set("id", {msg: createMessage("id"), retryCount: 0, nextAlarm: Date.now() + 1000});
      queue.routeInFailedState.set("id2", {msg: createMessage("id2"), retryCount: 0, nextAlarm: Date.now() + 500});
      await queue.alarm();
      expect(queue.routeInFailedState.size).toBe(1);
      expect(queue.service.fetch).toHaveBeenCalledTimes(1);
