...
`clearMocks` and `restoreMocks` are now enabled globally in `vitest.config.ts`, removing the need for per-test `vi.clearAllMocks()` and `vi.restoreAllMocks()` calls across test files. An explicit `vi.spyOn(AbortSignal, "timeout")` was added to tests that previously relied on spy state leaking from a preceding test — a leak that `restoreMocks` now correctly prevents.
